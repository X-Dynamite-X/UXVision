...
interface PaginatedUsers {
    data: User[]; // data يجب أن يكون مصفوفة عادية
    prev_page_url?: string | any;
    next_page_url?: string | any;
    current_page?: number;
    last_page?: number;
}

const page = usePage<{ users: PaginatedUsers }>();
...
const openDeleteModel = (data: ModelData | any) => {
    console.log(data);
    showDeleteModel.value = true;
    modelData.value = { ...data };
};

const deleteData = (data: ModelData | any) => {
...
watch(
    search,
    debounce(function (newValue: any) {
        router.get(
            '/admin/users',
            {
                search: newValue,
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4">
      ...
      >
        ...
        <div
          class="flex-column flex-wrap items-center justify-between space-x-4 space-y-4 pb-4 sm:flex-row sm:space-y-0"
        >
          ...
          <DynamicDelete
            ...
            :data="modelData"
          >
            <template #message="{ data }: { data: User | any }">
              <Label class="text-lg">
